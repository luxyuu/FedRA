...
class part():
    ...
    def pathname(self):
        # 保存训练结果
        base_dir = f"./logs_FL/14BUS/part{self.num}/14exp"
    ...
    def train(self):
        ...
        if self.commu > 0:
            ...
            model.load_state_dict(torch.load(f"logs_FL/14BUS/pth/model_14exp{self.counterr}_{self.commu}.pth"),strict=False)
...
# FL
    ...
    for i in range(26):                          
        ...
        model_paths = [f'./logs_FL/14BUS/part{j}/14exp{counter}/final{i+1}.pth' for j in range(1, N+1)]
        print('进行-第%d轮' % (i+1))
        
        print('聚合和平均模型参数ing')
        save_path = f"./logs_FL/14BUS/pth/model_14exp{counter}_{i+1}.pth"
