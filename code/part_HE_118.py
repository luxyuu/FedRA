...
class part():
    ...
    def pathname(self):
        # 保存训练结果
        base_dir = f"./logs_FL_HE/118BUS/part{self.num}/118exp"
    ...
    def train(self):
        ...
        if self.commu > 0:
            ...
            model.load_state_dict(torch.load(f"logs_FL_HE/118BUS/pth/model_118exp{self.counterr}_{self.commu}.pth"),strict=False)
...
# FL_HE
    ...
    for i in range(26):                          
        ...
        model_paths = [f'./logs_FL_HE/118BUS/part{j}/118exp{counter}/final{i+1}.pth' for j in range(1, N+1)]
        print('进行-第%d轮' % (i+1))
        # 使用加密器聚合和平均模型参数
        ...
        # 指定解密后模型的保存路径，并保存
        print('解密后模型并保存ing')
        save_path = f"./logs_FL_HE/118BUS/pth/model_118exp{counter}_{i+1}.pth"
